...
export const CadastroBarbearia = () => {
      ...
      if (cepFormatado.length === 8) {
        fetchCEPData(cepFormatado).then((data) => {
          if (data) {
            ...
            const ufEncontrado = estados.find(
  ...
  const validateForm = () => {
    const validations = [
      {
        condition: !validateEmail(emailInput),
        message: "Por favor, insira um email válido.",
      },
      {
        condition: nameInput.trim() === "",
        message: "Nome não pode estar vazio.",
      },
      {
        condition: phoneInput.trim() === "",
        message: "Telefone não pode estar vazio.",
      },
      {
        condition: !/^\(\d{2}\) \d{4,5}-\d{4}$/.test(phoneInput),
        message:
          "Telefone deve estar no formato (XX) XXXX-XXXX ou (XX) XXXXX-XXXX.",
      },
      {
        condition: !validatePassword(passwordInput),
        message:
          "A senha deve ter pelo menos 8 caracteres, incluindo letras maiúsculas, minúsculas, números e caracteres especiais.",
      },
      {
        condition: passwordInput !== confirmPasswordInput,
        message: "As senhas não coincidem.",
      },
      {
        condition: streetInput.trim() === "",
        message: "Endereço não pode estar vazio.",
      },
      {
        condition: numInput.trim() === "",
        message: "Número não pode estar vazio.",
      },
      {
        condition: bairroInput.trim() === "",
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      if (descInput && descInput.trim())
        formData.append("descricao", descInput);
    ...
    <div className="bg-[#24211c] min-h-screen w-screen flex justify-center items-center bg-gradient-to-b from-black/90 to-black/40">
      <div className="bg-white w-screen h-auto md:h-[580px] rounded-md overflow-hidden shadow-sm md:w-[95vw] flex flex-col md:flex-row lg:flex-row-reverse justify-start lg:w-[900px] transition-all">
        ...
        <div className="p-3 flex flex-col justify-center items-center text-center md:w-[60%] lg:w-[55%]">
          <h2 className="text-2xl font-bold mb-1 md:text-2xl lg:text-3xl">
            Code & Shave 💈
          </h2>
          ...
          <h1 className="font-bold text-lg md:text-xl lg:text-2xl mt-2 mb-2">
            Cadastro - Barbearia
          </h1>
          <div className="overflow-y-auto py-2">
            ...
            >
              ...
              <div className="flex flex-col">
                <label className="text-left text-gray-500 text-sm">
                  Descrição da barbearia (opcional)
                </label>
                <textarea
                  rows={3}
              ...
              <div className="flex flex-col md:flex-row items-start justify-between gap-3">
                <div className="flex flex-col w-[50px]">
                  ...
                  <select
                    value={ufSelect.sigla || ""}
                    className="text-gray-500"
                    onChange={(e) => handleUF(e)}
                  >
                ...
                <div className="flex flex-col flex-1 w-full">
                  ...
                  <select
                    value={citySelect}
                    className="w-full shadow-lg outline-none text-gray-500"
                    onChange={(e) => setCitySelect(e.target.value)}
                  >
                    <option value="" disabled></option>
                    {cidadesEstado &&
                      cidadesEstado.map((item, index) => (
              ...
              <div className="flex flex-col">
                <label className="text-left text-gray-500 text-sm rounded-md">
                  Senha
                </label>
                <div className="shadow-lg p-2 w-full flex items-center">
                  ...
                  <button
                    onClick={(e) =>
                      switchTypePassword(e, typePassword, setTypePassword)
                    }
                  >
